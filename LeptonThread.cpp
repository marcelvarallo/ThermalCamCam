...
int threaded = 0;
int readyToToggle = 1;
const int *colSet = colormap_ironblack;
...
while(threaded==0 || threaded ==1) {
	SpiOpenPort(0);
	while(true && threaded==0) {
...
void LeptonThread::colorChange() {
        if (colSet == colormap_ironblack) {
		colSet = colormap_rainbow;
	}
	else if (colSet == colormap_grayscale) {
		colSet = colormap_ironblack;
	}
	else {
		colSet = colormap_grayscale;
	}
}
void LeptonThread::webcam() {
	threaded = 3;
}
void LeptonThread::rebootMe() {
	if (threaded ==0) {
		threaded = 1 ;
	}
	else {
